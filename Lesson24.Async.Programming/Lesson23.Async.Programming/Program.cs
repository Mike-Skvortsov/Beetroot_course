...
namespace Lesson23.Async.Programming
{
	class Program
	{
		static async Task Main(string[] args)
		{
			...
			var requestMessageWeather = new HttpRequestMessage
			{
				//достаю посилання на файл
				RequestUri = new Uri("https://goweather.herokuapp.com/weather/Curitiba")
			};
			var responseMessageWeather = await httpClient.SendAsync(requestMessageWeather);
			var strResponceWeather = await responseMessageWeather.Content.ReadAsStringAsync();
			var temperature = JsonConvert.DeserializeObject<Weather>(strResponceWeather);
			var wind = JsonConvert.DeserializeObject<Weather>(strResponceWeather);
			Console.WriteLine($"temperature is {temperature!.Temperature} and wind {wind!.Wind}");
			await using var streamWeather = await imageResponse.Content.ReadAsStreamAsync();
			await using var fileStreamWeather = File.Create("Weather.txt");
			streamWeather.Seek(0, SeekOrigin.Begin);
			await streamWeather.CopyToAsync(fileStreamWeather);
			//End homework
		}
