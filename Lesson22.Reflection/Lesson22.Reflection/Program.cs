...
namespace Lesson22.Reflection
{
	class Program
	{
		static void Main(string[] args)
		{
			...
			Type vehicleTypeCar = typeof(Car);
			Console.WriteLine(vehicleType);
			Console.WriteLine(vehicleType.Attributes);
			foreach (PropertyInfo propertyInfo in vehicleType.GetProperties())
			{
				Console.WriteLine(propertyInfo);
			}
			foreach (MethodInfo methodInfo in typeof(Vehicle).GetMethods())
			{
				Console.WriteLine($"{vehicleType} have {methodInfo} methods");
			}
			Console.WriteLine("__________________________________________________");
			foreach (MethodInfo item in typeof(Car).GetMethods())
			{
				Console.WriteLine($"{vehicleTypeCar} have methods which return {item.ReturnType.FullName} type");
			}
			Console.WriteLine(vehicleType.IsPublic);
			Console.WriteLine(vehicleTypeCar.GetProperties().Length);
			Console.WriteLine(vehicleTypeCar.FullName == vehicleTypeCar.Name);
			Console.WriteLine(vehicleType.BaseType == vehicleTypeCar.BaseType);
			Console.WriteLine(vehicleType.GetRuntimeMethods().Last());
			Console.WriteLine(vehicleTypeCar.AssemblyQualifiedName);
		}
